...
export class Runner {
    ...
    public async run() {
        ...
        progressBar.start(this.numProjects, 0);
        while (!this.isDone()) {
            this.simulate();
            progressBar.update(this.numProjects - this.projectsArr.length);
        }
        progressBar.update(this.numProjects);
        progressBar.stop();
    }
    ...
    public async simulate() {
        ...
        //do a pass to make assingments with as many mentors as possible
        this.projectsArr.forEach(el => {
